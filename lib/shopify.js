  ...
  const query = gql`
    {
      metaobject(id: "gid://shopify/Metaobject/57147582") {
        ...
        featured_influencers: field(key: "24_featured_influencers") {
          key
          value
        }
...
export async function getCollections(collectionType) {
  const query = gql`
    {
      collections(first: 200) {
        edges {
          node {
            ...
            metafields(
              identifiers: [
                { namespace: "custom_fields", key: "collection_type" }
              ]
            ) {
              key
              value
  ...
  response.collections.edges.map((c) => console.log(c.node.metafields[0]));

  // Remove collections without products
  return response.collections.edges.filter(
    (collection) =>
      collection.node.metafields[0] &&
      collection.node.metafields[0].value === collectionType
  );
}
