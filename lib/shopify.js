        ...
        products(first: 12) {
          edges {
            node {
              ...
              meta.custom.influencer: metafield(namespace: "custom", key: "influencer") {
                value
                }
              }
              images(first: 5) {
...
export async function updateUserNameReq(userInfo) {
  const input = {
    customer: {
  ...
  const response = await GraphQLRequest(query, input, "updateUserNameReq");
